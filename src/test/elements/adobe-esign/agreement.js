...
suite.forElement('esignature', 'agreements', { payload: payload }, (test) => {
  ...
  it('should allow S for /hubs/esignature/agreements-asset-events', () => {
    return cloud.withOptions({ qs: { where: `startDate = '${startDate}' and endDate = '${currentDate}'` } }).get(`/hubs/esignature/agreement-asset-events `);
  });

  it('should allow CRDS for /hubs/esignature/agreements and sub-resources', () => {
    return cloud.postFile(`/hubs/esignature/transient-documents`, `${__dirname}/assets/attach.txt`)
