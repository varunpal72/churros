...
const payload = require('./assets/dimension-properties');
const build = (overrides) => Object.assign({}, payload, overrides);
const dimensionPropertiesPayload = build({Name: "name" + tools.randomInt()});

suite.forElement('finance', 'dimension-properties',{ payload:dimensionPropertiesPayload } , (test) => {
  let name,id,updatedPayload;
  it('should support GET Dimension', () => {
    return cloud.get(`/hubs/finance/dimensions`)
      .then(r => dimensionPropertiesPayload.Dimension = r.body[0].id);
    });
