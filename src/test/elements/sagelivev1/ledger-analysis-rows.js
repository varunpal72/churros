...
const payload = tools.requirePayload(`${__dirname}/assets/ledger-analysis-rows.json`);
...
suite.forElement('finance', 'ledger-analysis-rows', { payload: payload }, (test) => {
  let id;
  const options = {
    churros: {
  ...
  before(() => cloud.post(`/hubs/finance/ledger-analyses`, prePayload)
    .then(r => {
      payload.LedgerAnalysis = r.body.id;
      id = r.body.id;
    }));
