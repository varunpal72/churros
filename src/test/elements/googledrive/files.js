...
suite.forElement('documents', 'files', { payload: payload }, (test) => {
  ...
  it('it should allow RS for documents/files/:id/revisions', () => {
    const revisionChecks = (jpgFileBody) => {
      ...
      return cloud.get(`${test.api}/${jpgFileBody.id}/revisions`)
        ...
        .then(r => expect(r.body.fileName).to.equal("Penguins.jpg"));
    };
  ...
  it('it should allow RS for documents/files/revisions by path', () => {
    const revisionChecks = (jpgFileBody) => {
        let revisionId;
      return cloud.withOptions({ qs: { path: `/${directoryPath}/Penguins.jpg`}}).get(`${test.api}/revisions`)
      .then(r => {
        expect(r.body.filter(obj => obj.fileName === 'Penguins.jpg')).to.not.be.empty;
        revisionId = r.body[0].id;
      })
      .then(() => cloud.withOptions({ qs: { path: `/${directoryPath}/Penguins.jpg`}}).get(`${test.api}/revisions/${revisionId}`))
      .then(r => expect(r.body.fileName).to.equal("Penguins.jpg"));
    };
    return fileWrap(revisionChecks);
  });
