...
tester.for(null, 'elements', metadataSchema, (api) => {
  ...
  it('should return element metadata', () => {
    return tester.get('elements/sfdc')
      .then(r => tester.get(util.format('%s/%s/metadata', api, r.body.id), metadataSchema));
  });

  it('should return 404 for invalid element ID', () => {
    return tester.get(util.format('%s/999999999999/metadata', api), (r) => expect(r).to.have.statusCode(404));
  });
