...
const updateInstanceWithReprovision = (baseUrl, schema) => {
  let id;
  return provisioner.create('hubspot')
    .then(r => id = r.body.id)
    .then(r => cloud.get(`${baseUrl}/${id}`, (r) => {
      expect(r).to.have.schemaAnd200(schema);
      expect(r.body.configuration).to.not.be.empty;
    }))
    .then(r => provisioner.partialOauth('hubspot'))
    .then(code =>
      cloud.put(`${baseUrl}/${id}`, genInstance('hubspot', { name: 'updated-instance', providerData: { code: code } }), r => {
        expect(r.body.configuration).to.not.be.empty;
...
suite.forPlatform('elements/instances', opts, (test) => {
  ...
  before(() => {
    return provisioner.create('hubspot')
      .then(r => hubspotId = r.body.id);
  });

  after(() => {
    return hubspotId ? provisioner.delete(hubspotId) : true;
  });
  ...
  it('should support using filter.response.nulls config to filter out or display nulls', () => {
    ...
    const validateNullsPresent = (shouldHaveNulls) => {
      return cloud.get('/hubs/marketing/files?pageSize=1')
        .then(r => {
    ...
    return cloud.get(`/instances/${hubspotId}/configuration`)
      .then(r => validate(r))
      .then(r => {
        configuration = r;
      ...
      .then(r => cloud.patch(`/instances/${hubspotId}/configuration/${configuration.id}`, Object.assign({}, configuration, { propertyValue: 'false' })))
      .then(r => cloud.get(`/instances/${hubspotId}/configuration/${r.body.id}`))
  ...
  it('should support get instance specific docs', () => {
    return cloud.post(`instances/${hubspotId}/objects/myaccounts/definitions`, objDefPayload)
      .then(r => setTimeout( () => cloud.post(`instances/${hubspotId}/transformations/myaccounts`, transformationPayload), 500))
      .then(r => cloud.get(`instances/${hubspotId}/docs`, sfdcSwaggerSchema));
  });
  ...
  it('should support updating the configuration for an element instance', () => {
    ...
    return cloud.get(`/instances/${hubspotId}/configuration`)
      .then(r => validate(r))
      .then(configuration => cloud.patch(`/instances/${hubspotId}/configuration/${configuration.id}`, Object.assign({}, configuration, { propertyValue: 'true' })))
      .then(r => cloud.get(`/instances/${hubspotId}/configuration/${r.body.id}`))
  ...
  it('should support updating the tags for an element instance', () => {
    ...
    let id;
    return provisioner.create('hubspot', { name: 'churros-test' })
      .then(r => id = r.body.id)
      .then(r => cloud.patch(`/instances/${id}`, { id: id, tags: ['churros-testing'] }))
      .then(r => cloud.get(`/instances/${id}`, validateTags(r.body.tags)))
  ...
  it('should support switching an instance to the clone of an element', () => {
    ...
    return provisioner.create('hubspot')
      ...
      .then(r => cloud.post('elements/hubspot/clone'))
      .then(r => clone = r.body)
      .then(r => cloud.patch(`instances/${instance.id}`, { element: { id: clone.id } })
        .then(r => {
          expect(r.body.element.id).to.equal(clone.id);
        }))
      .then(r => provisioner.delete(instance.id, 'elements/hubspot/instances'))
      .then(r => cloud.delete(`elements/${clone.key}`));
  });

  it('should sanitize element instance name on create and update', () => {
    let id;
    return provisioner.create('hubspot', { name: '<a href="#" onClick="javascript:alert(\'xss\');return false;">churros-xss</a>' })
      .then(r => id = r.body.id)
  ...
  it('should fail with 401 for deleted instance api call', () => {
    ...
    return provisioner.create('hubspot')
      ...
      .then(() => cloud.get('/hubs/marketing/contacts', (r) => expect(r).to.have.statusCode(401)));
  });
});
