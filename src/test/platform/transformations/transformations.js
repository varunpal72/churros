...
const crud = (url, payload, updatePayload, schema) => {
  return cloud.post(url, payload, schema).then(r => cloud.get(url, schema)).then(r => cloud.put(url, updatePayload, schema)).then(r => cloud.delete(url));
};
...
const crudTransformsByName = (level, elementKey, payload, updatePayload, schema) => {
  let objectName = "churros-object-" + tools.random();
  return cloud.post(getObjectDefUrl(level, objectName), genDefaultObjectDef({})).then(r => crud(getTransformUrl(level, objectName, elementKey), payload, updatePayload, schema)).then(r => cloud.delete(getObjectDefUrl(level, objectName)));
};
...
const testTransformationForInstance = (objectName, objDefUrl, transUrl) => {
  return cloud.post(objDefUrl, genDefaultObjectDef({}))
    ...
    .then(r => cloud.post(transUrl, genDefaultTrans({}))).then(r => cloud.get("hubs/crm/" + objectName, r => {
    ...
    // test remove config
    .then(r => cloud.put(transUrl, genTransWithRemove({}))).then(r => cloud.get("hubs/crm/" + objectName, r => {
      expect(r.body).to.not.be.empty;
      r.body.forEach(item => {
    ...
    .then(r => cloud.put(transUrl, genTransWithPassThrough({}))).then(r => cloud.get("hubs/crm/" + objectName, r => {
      expect(r.body).to.not.be.empty;
      r.body.forEach(item => {
        expect(item.churrosMod).to.be.empty;
      });
    })).then(r => cloud.delete(transUrl)).then(r => cloud.delete(objDefUrl));
};
...
suite.forPlatform("transformations", {
  schema: schema
}, test => {
  ...
  it("should support account-level object definition CRUD by name", () => {
    let accountId;
    return getPlatformAccounts().then(r => r.body.forEach(account => accountId = account.defaultAccount ?
      (accountId = account.id) :
      accountId)).then(r => crudObjectDefsByName("accounts/" + accountId, genDefaultObjectDef({}), genDefaultObjectDef({}), objDefSchema));
  });
  it("should support account-level transformation CRUD by name and element key", () => {
    let accountId;
    return getPlatformAccounts().then(r => r.body.forEach(account => accountId = account.defaultAccount ?
      (accountId = account.id) :
      accountId)).then(r => crudTransformsByName("accounts/" + accountId, elementKey, genDefaultTrans({}), genDefaultTrans({}), schema));
  });
  it("should support account-level transformation CRUD by name and element ID", () => {
    let accountId;
    return getPlatformAccounts().then(r => r.body.forEach(account => accountId = account.defaultAccount ?
      (accountId = account.id) :
      accountId)).then(r => crudTransformsByName("accounts/" + accountId, elementId, genDefaultTrans({}), genDefaultTrans({}), schema));
  });
  it("should support account-level transformations", () => {
    ...
    return getPlatformAccounts().then(r => r.body.forEach(account => accountId = account.defaultAccount ?
      (accountId = account.id) :
      accountId)).then(r => level = "accounts/" + accountId).then(r => testTransformation(sfdcId, objectName, getObjectDefUrl(level, objectName), getTransformUrl(level, objectName, elementKey)));
  });
  /** instance-level */
  ...
  it("should support transformation inheritance", () => {
    let objectName = "churros-object-" + tools.random();
    let accountId;
    return cloud.post(getObjectDefUrl("organizations", objectName), genBaseObjectDef({})).then(r => cloud.post(getTransformUrl("organizations", objectName, elementKey), genBaseTrans({}))).then(r => cloud.get(`hubs/crm/${objectName}`, r => {
      ...
      .then(r => getPlatformAccounts()).then(r => r.body.forEach(account => accountId = account.defaultAccount ?
        (accountId = account.id) :
        accountId)).then(r => {
      ...
      .then(r => {
        ...
        let trans = genBaseTrans({
          fields: [getTransField("churrosMod", "string", "LastModifiedDate", "string")],
          configuration: [getConfig("inherit", true, true)]
        });
      ...
      // clean up!
      .then(r => cloud.delete(getTransformUrl("instances/" + sfdcId, objectName))).then(r => cloud.delete(getObjectDefUrl("instances/" + sfdcId, objectName))).then(r => cloud.delete(getTransformUrl("accounts/" + accountId, objectName, elementKey))).then(r => cloud.delete(getObjectDefUrl("accounts/" + accountId, objectName))).then(r => cloud.delete(getTransformUrl("organizations", objectName, elementKey))).then(r => cloud.delete(getObjectDefUrl("organizations", objectName)));
  });
});
