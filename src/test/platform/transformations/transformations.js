...
const genObjectDefs = () => new Object({
  'churros-object-1': genDefaultObjectDef({ objectName: 'churros-object-1'}),
  'churros-object-2': genDefaultObjectDef({ objectName: 'churros-object-2'})
});
...
const getTransField = (path, type, vendorPath, vendorType, configuration) => {
  return { path: path, type: type, vendorPath: vendorPath, vendorType: vendorType, configuration: configuration }
};
const addTransField = (trans, path, type, vendorPath, vendorType, configuration) => {
  trans.fields.push(getTransField(path, type, vendorPath, vendorType, configuration));
};
