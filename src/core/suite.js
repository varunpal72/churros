...
const requiredFields = (api, pay, options, resource) => {
  ...
  boomGoesTheDynamite('Required Fields:', () => {
    ...
    .then(() => {
      const element = tools.getBaseElement(argv.element);
      const path = `${__dirname}/../test/elements/${element}/assets/${resource}.json`;
      const ogPayload = require(path);
      let payloads = tools.keysOnLevel(ogPayload);
...
const runTests = (api, payload, validationCb, tests, hub) => {
  const should = (api, validationCb, payload, options, name, hub) => ({
    ...
    */
    getRequiredFields: (resource) => requiredFields(api, payload, options, resource),
    /**
...
const run = (api, resource, options, defaultValidation, tests, hub) => {
  ...
  if (options.skip) {
    ...
    argv.requiredFields ? describe(name, () => runTests(api, options.payload, defaultValidation, test => test.should.getRequiredFields(resource), hub)) :
