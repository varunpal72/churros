...
const validator = (validationCb) => {
  if (typeof validationCb === 'function') {
    return (r) => {
      logger.debug(`Validating response against validation callback.  Response: ${tools.stringify(r.body)}`);
      validationCb(r);
      return r;
    };
  } else if (typeof validationCb === 'undefined' || (typeof validationCb === 'object' && validationCb === null)) {
    return (r) => {
      logger.debug(`Validating that response is 200.  Response: ${tools.stringify(r.body)}`);
    ...
    return (r) => {
      logger.debug(`Validating response against JSON schema. Response: ${tools.stringify(r.body)}`);
